...
function responseError(status, detail) {
  ...
  if (status === 429) return new Error('智谱请求过于频繁，请稍后再试')
  if (status >= 500) return new Error('智谱服务暂时不可用')
  return new Error(detail || `智谱请求失败（HTTP ${status}）`)
}
...
async function chat({ apiKey, model, messages, maxTokens = 320, signal }) {
  ...
  if (typeof text !== 'string' || !text.trim()) throw new Error('智谱没有返回有效回复')
  return {
    text: text.trim(),
