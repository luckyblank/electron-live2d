...
当前插件管理器假定插件调用需要 API Key。凭据来源有三种，按以下顺序解析：

1. 项目根 `.env` 中由清单 `apiKeyEnv` 指定的非空值。
2. 当前进程中同名的系统环境变量。
3. 旧版本在设置页输入并由 Electron `safeStorage` 加密保存的本地 Key。

`.env` 中该项为空或不存在时才回退系统环境变量；只要前两项任一可用，就不会被本机加密值覆盖。保留第三项仅为兼容已有用户配置。`.env` 与 `.env.*` 必须加入 Git 排除规则，正式打包只排除项目根 `.env`，不对依赖包中的同名文件做全局过滤。配置文件或系统环境变量发生变化后需要完整重启应用。

开发规范：

- 环境变量名使用服务商前缀，例如 `MY_PROVIDER_API_KEY`。
- 在 `.env.example` 中只保留变量名和空值，不得提交真实 Key。
